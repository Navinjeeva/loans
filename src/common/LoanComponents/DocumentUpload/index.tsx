 ...
 * />
 *
 * // With flat details for manual preview (click eye icon to view)
 * <DocumentUpload
 *   header="Beneficiary"
 *   images={uploadedImages}
 *   details={{
 ...
 * />
 *
 * // With nested object details (automatically flattened for display)
 * <DocumentUpload
 ...
 *   }}
 *   setImages={setUploadedImages}
 *   // Nested objects are flattened and displayed as "Name > First", "Address > City", etc.
 * />
 *
 * // With arrays of objects (automatically flattened with indices)
 * <DocumentUpload
 *   header="Beneficiaries"
 *   images={uploadedImages}
 *   details={{
 *     "beneficiaries": [
 *       { "name": "John Doe", "relation": "Brother" },
 *       { "name": "Jane Smith", "relation": "Sister" }
 *     ],
 *     "tags": ["urgent", "verified"] // Arrays of primitives shown as comma-separated
 *   }}
 *   setImages={setUploadedImages}
 *   // Array objects displayed as "Beneficiaries #1 > Name", "Beneficiaries #2 > Name", etc.
 * />
 */
  ...
  Object.entries(obj).forEach(([key, value]) => {
    ...
    if (
      ...
      if (value.length === 0) {
        ...
        // Array contains objects - flatten each object with index
...
// Helper function to format nested keys for display
    ...
    .map((part) => {
      ...
      if (arrayMatch) {
        ...
        const formattedName = name
          ...
          .join(" ");
        return `${formattedName} #${parseInt(index) + 1}`;
      }

      // Regular key formatting
      return part
        .replace(/_/g, " ")
        .split(" ")
        .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
...
const DetailsModal = ({
  ...
  // Flatten nested objects for rendering
  const flattenedDetails = React.useMemo(() => {
    return flattenObject(isEditing ? editedDetails : details);
  }, [isEditing, editedDetails, details]);

  // Convert details object to array for better rendering
  const detailsArray = Object.entries(flattenedDetails);
          ...
          >
            ...
            >
              ...
              {detailsArray.length > 0 ? (
                ...
                >
                  <LottieView
                    source={require("@src/common/animation/extracting-animation.json")}
                    autoPlay
                    loop
                    resizeMode="cover"
                    style={{
...
const CustomUploadButton = ({
  ...
  showPreviewButton = true, // Controls whether preview button is shown
  onDetailsUpdate, // Callback when details are updated from modal
  expired = false, // Whether the document has expired
}: {
  header?: string;
  headerDesc?: string;
  title?: string;
  ...
  showPreviewButton?: boolean; // Controls whether preview button is shown
          ...
          <View style={styles.detailsInfo}>
            <View style={styles.detailsTextContainer}>
              {(details?.full_name ||
                ...
                details?.["driver name"] ||
                details?.["FULL NAME"] ||
                details?.customer_info?.customer_name) && (
              ...
              {(details?.card_type_english ||
                details?.["CARD TYPE ENGLISH"] ||
                details?.card_type ||
                details?.["CARD TYPE"] ||
                details?.document_type ||
                details?.["DOCUMENT TYPE"] ||
                details?.document_info?.type) && (
          ...
          {expired === true && (
            <View>
              ...
              <View
                style={{
                  height: 1,
                  backgroundColor: colors.error || "#F13937",
                  marginVertical: hp(1.5),
                }}
              />

              {/* Expired warning message with action buttons */}
              <View
                style={{
                  flexDirection: "row",
                  alignItems: "center",
                  ...
                  <TouchableOpacity
                    ...
                    onPress={async () => {
                      ...
                      if (results.length > limit) {
                        ...
                        // PersonalDoc will handle replace vs append based on expired flag
                        setImages(files);
                      }
                    }}
                    style={[
